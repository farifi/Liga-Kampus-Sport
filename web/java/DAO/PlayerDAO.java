...
public class PlayerDAO {
    ...
    public void insertPlayer(Player player) {
        String sql = "INSERT INTO players (TEAM_ID, SPORT_ID, PLAYER_NAME, STUDENT_ID, JERSEY_NO, POSITION) " +
    ...
    public void updatePlayer(Player player) {
        String sql = "UPDATE players SET PLAYER_NAME = ?, STUDENT_ID = ?, JERSEY_NO = ?, POSITION = ? WHERE PLAYER_ID = ?";
        try (Connection conn = DBConnection.getConnection()) {
            if (conn == null) return;
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
