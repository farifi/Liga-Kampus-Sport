...
public class TeamDAO {
    ...
    public void insertTeam(Team team) {
        String sql = "INSERT INTO team (SPORT_ID, COMPETITION_ID, TEAM_NAME, COACH_NAME, FACULTY, MANAGER_ID) VALUES (?, ?, ?, ?, ?, ?)";
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                ...
                if (team.getManager() != null && team.getManager().getUserId() > 0) {
                    ps.setInt(6, team.getManager().getUserId());
                } else {
    ...
    public void updateTeam(Team team) {
        String sql = "UPDATE team SET SPORT_ID = ?, COMPETITION_ID = ?, TEAM_NAME = ?, COACH_NAME = ?, FACULTY = ?, MANAGER_ID = ? WHERE TEAM_ID = ?";
        try (Connection conn = DBConnection.getConnection()) {
            if (conn == null) return;
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setInt(1, team.getSport().getSportId());
